...
import 'package:untitled/pages/classes.dart';
  ...
  Widget square(j){
    return ElevatedButton(onPressed:(){
      if(submit){
        ...
        b![j.i][j.j]!.pressed=1;
        Navigator.pushReplacementNamed(context, '/game',arguments:{'b1':b,'b2':b2,'sub':false,'s':j.i,'t':j.j,'meh':true,'limit1':limit1,'limit2':limit2});
      }
    },
        style:ElevatedButton.styleFrom(
          padding: EdgeInsets.zero,
  ...
  image(i,j) {
    if (submit) {
      if (b2[i][j]!.ro != null) {
        ...
        if (rotate) {
          if (b2[i][j]!.pos == 0) {
            b2[i][j]!.ro = 'ships30.jpg';
            return b2[i][j]!.ro;
          }
          else if (b2[i][j]!.pos == 1) {
            b2[i][j]!.ro = 'ships20.jpg';
            return b2[i][j]!.ro;
          }
          else if (b2[i][j]!.pos == 2) {
            b2[i][j]!.ro = 'ships10.jpg';
            return b2[i][j]!.ro;
          }
          else {
            return 'water.jpg';
          ...
          if (b2[i][j]!.pos == 0) {
            b2[i][j]!.ro = 'ships11.jpg';
            return b2[i][j]!.ro;
          }
          else if (b2[i][j]!.pos == 1) {
            b2[i][j]!.ro = 'ships21.jpg';
            return b2[i][j]!.ro;
          }
          else if (b2[i][j]!.pos == 2) {
            b2[i][j]!.ro = 'ships31.jpg';
            return b2[i][j]!.ro;
          }
          else {
            return 'water.jpg';
    ...
    return Scaffold(
      body: Column(
        children: [
          ...
          ElevatedButton(onPressed: (){
            if(submit){
              Navigator.pushNamed(context, '/game',arguments: {'b1':b,'b2':b2,'sub':false,'s':s,'t':t,'meh':false,'limit1':limit1,'limit2':limit2});
            }
          }, child: Text("submit"))
        ],
      ),
